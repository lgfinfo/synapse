...
impl ServiceClient {
    ...
    pub async fn subscribe(
        ...
        tokio::spawn(async move {
            // query service list before subscribe
            let req = QueryRequest::new(name.clone());
            match client.query_services(req).await {
                Ok(list) => {
                    let list = list.into_inner().services;
                    for service in list {
                        if let Err(e) = tx.send(service).await {
                ...
                Err(err) => {
                    error!("Failed to query service: {}", err);
                    return;
                }
            }
            let max_retries = 5;
